...
class TimeTrackingDataHandler(SafeHandler):
    ...
    def get(self):
        ...
        try:
            ...
            application_log.info(f"Starting time tracking query with date range {start_date} to {end_date}")
            ...
            application_log.debug(f"Retrieved {projects_examined} projects from time_tracking_delivered view")
            ...
            for row in projects_view:
                ...
                library_construction_method = value.get("library_construction_method", "Unknown")
                project_type = value.get("type", "Unknown")  # Field is "type" in the view
                    ...
                    if start_value and end_value:
                        try:
                            ...
                            if duration_days >= 0:
                                ...
                                method_data[stage_name]["durations"].append(duration_days)
                                method_data[stage_name]["projects"].append({
                                    ...
                                    "sequencing_platform": value.get("sequencing_platform"),
                                    "sequence_units_ordered": value.get("sequence_units_ordered"),
                                    "open_date": value.get("open_date"),
                                    "queued": value.get("queued"),
                                    "library_prep_start": value.get("library_prep_start"),
                                    "qc_library_finished": value.get("qc_library_finished"),
                                    "sequencing_start_date": value.get("sequencing_start_date"),
                                    "all_samples_sequenced": value.get("all_samples_sequenced"),
                                    "all_raw_data_delivered": value.get("all_raw_data_delivered"),
                                    "best_practice_analysis_completed": value.get("best_practice_analysis_completed")
                                })
