...
function select_from_preset(preset_type, preset, data=null) {
    if (data == null){
     $.getJSON('/api/v1/presets?presets_list=pv_presets', function(response){sel_from_ps(preset_type, preset, response); data=response;}).fail(function(jqXHR, textStatus, errorThrown) { alert('getJSON request failed! ' + textStatus); });
    }else{
