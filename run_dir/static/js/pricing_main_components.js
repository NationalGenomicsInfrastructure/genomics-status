...
export const vPricingMain = {
    ...
    data() {
        return {
            ...
            is_quote_or_agreement: this.$root.Origin ? true : false
        }
    },
    computed: {
        any_errors() {
        ...
        all_products_per_category() {
            let prod_per_cat = {};
            let cat = '';
            for (const [prod_id, product] of Object.entries(this.all_products)) {
            ...
            let sorted_products = [];
            for (let category in prod_per_cat) {
        ...
        product_categories() {
            ...
            let categories_array = Array.from(categories).sort()
            if (add_new_at_start) {
        ...
        all_components_per_category() {
            let comp_per_cat = {};
            let cat = '';
            for (const [comp_id, component] of Object.entries(this.all_components)) {
            ...
            for (const category in comp_per_cat) {
        ...
        next_product_id() {
            ...
            let all_ids = Object.keys(this.all_products)
            let max_id = Math.max(...all_ids.map(x => parseInt(x)))
            return (1 + max_id).toString()
        },
        next_component_id() {
            /* Returns the lowest unused component id */
            let all_ids = Object.keys(this.all_components)
            let max_id = Math.max(...all_ids.map(x => parseInt(x)))
            return (1 + max_id).toString()
        }
    },
    ...
    methods: {
        ...
        cloneProduct(prod_id) {
            let product = this.all_products[prod_id]
            ...
            let new_id = this.next_product_id
        ...
        removeProductComponent(product_id, component_id, type) {
            const reset_empty = function(components_object) {
        ...
        cloneComponent(comp_id) {
            let component = this.all_components[comp_id]
            let new_comp = Object.assign({}, component)
            let new_id = this.next_component_id
        ...
        fetchDraftCostCalculator(assign_data) {
            axios
                ...
                .then(response => {
                    let data = response.data.cost_calculator
                    if (data !== null) {
        ...
        fetchExchangeRates(date) {
            let url = '/api/v1/pricing_exchange_rates'
            if (date !== undefined) {
                url += '?date=' + date;
            }
            axios
        ...
        componentCost(comp_id) {
            ...
            let list_price = component['List price']
            let sek_list_price = 0
            let sek_price = 0
            let sek_price_per_unit = 0
            let currency_key = ''
            if (currency == 'SEK') {
        ...
        productCost(prod_id) {
            ...
            if (product['is_fixed_price']) {
                ...
                cost = 0
                for (const [comp_id, info] of Object.entries(product['Components'])) {
                    let componentCosts = this.componentCost(comp_id)
                    let quantity = info['quantity']
                    cost += quantity * componentCosts['sek_price_per_unit']
                }
                cost_academic = cost + cost * product['Overhead']

                let full_cost_fee = parseFloat(product['Full cost fee']) || 0
        ...
        assignNewItems() {
             ...
             */
            let draft_prod_ids = new Set(Object.keys(this.all_products))
            let draft_comp_ids = new Set(Object.keys(this.all_components))

            let published_prod_ids = new Set(Object.keys(this.published_cost_calculator['products']))
            let published_comp_ids = new Set(Object.keys(this.published_cost_calculator['components']))

            let new_prod_ids = this.symmetricSetDifference(draft_prod_ids, published_prod_ids)
            let new_comp_ids = this.symmetricSetDifference( draft_comp_ids, published_comp_ids)
        ...
        populatedComponents(product_id, type) {
            ...
            for (const [comp_id, info] of Object.entries(component_input)) {
                components[comp_id] = {
    ...
    template: 
    `        
        <v-pricing-quote v-if="is_quote_or_agreement" :origin="this.$root.Origin" :is_proj_coord="this.$root.is_proj_coord"></v-pricing-quote>
...
export const vExchangeRates = {
  ...
  methods: {
    reload_exch_rates() {
        let date = this.issued_at_form_bound;
...
export const vProductTableRowTd = {
    /* Indivudal <td> for product table (not all of them) */
    name: 'v-product-table-row-td',
    props: ['td_key', 'row_changes', 'product_id', 'quotable'],
    components: {
        'v-product-table-components': vProductTableComponents,
    },
    data() {
        return { tooltip: null }
    },
    computed: {
        ...
        tooltip_html() {
            if (this.is_changes) {
                let return_html = '<div class="row pricing-td-highlight-tooltip"><div class="col-5">'
                const arrow_element = '</div><div class="col-2 d-flex align-self-center px-0"><h5><i class="fas fa-arrow-right"></i></h5></div>'
                if (typeof this.changes[0] == 'string') {
                    ...
                    for (const [prod_id, quantity_data] of Object.entries(this.changes[1])) {
                        return_html += '<p>' + quantity_data['quantity'] + ' x ' + this.$root.all_components[prod_id]['Product name'] + '</p>'
                    }
                    return_html += arrow_element
                    return_html += '<div class="col-5">'
                    for (const [prod_id, quantity_data] of Object.entries(this.changes[0])) {
                        return_html += '<p>' + quantity_data['quantity'] + ' x ' + this.$root.all_components[prod_id]['Product name'] + '</p>'
                    }
                    return_html += '</div></div>'
                    return return_html
          ...
          <template v-else>
            <template v-if="this.td_key == 'Cost'">
                <ul v-if="quotable===false || this.$root.price_type ==='cost_academic'" class="list-group list-group-horizontal">
                    <li class="list-group-item col-6">Academic</li>
                    <li class="list-group-item col-5">{{ cost['cost_academic'].toFixed(2) }}</li>
                </ul>
                <ul v-if="quotable===false || this.$root.price_type ==='full_cost'" class="list-group list-group-horizontal">
                    <li class="list-group-item col-6">Industry and Non Swedish Academia</li>
                    <li class="list-group-item col-5">{{ cost['full_cost'].toFixed(2)  }} </li>
                </ul>
                <ul v-if="quotable===false || this.$root.price_type ==='cost'" class="list-group list-group-horizontal">
                    <li class="list-group-item col-6">Internal</li>
                    <li class="list-group-item col-5">{{ cost['cost'].toFixed(2) }}</li>
                </ul>
            </template>
            <template v-else>
                <p>{{product[td_key]}}</p>
...
export const vProductTableRow = {
    /* Individual rows of the product table. */
    name: 'v-product-table-row',
    props: ['product_id', 'quotable'],
    components: {
        'v-product-table-row-td': vProductTableRowTd,
    },
    computed: {
        product() {
            return this.$root.all_products[this.product_id]
...
export const vProductsTable = {
    /* Table listing products - either draft or published.
     * Can optionally:
     *   - show discontinued products
     *   - have buttons to add products to quote
     */
    name: 'v-products-table',
    props: ['show_discontinued', 'quotable', 'refresh_table', 'id_add'],
    components: {
        'v-product-table-row': vProductTableRow,
    },
    data: function() {
        return {
            dataTable: null,
            table_id: 'pricing_products_table'
        }
    },
    mounted() {
        this.$nextTick(() => {
    ...
    methods: {
        ...
        init_datatable() {
            ...
            if(this.id_add){
                if(!this.table_id.includes(this.id_add)){
                    this.table_id = this.table_id + '_' +this.id_add;
                }
            }
            ...
            $('#'+this.table_id+'_filter').addClass('col-md-2');
            $('#'+this.table_id+'_filter').appendTo("#table_h_and_search");
            $('#'+this.table_id+'_filter label input').appendTo($('#'+this.table_id+'_filter'));
            $('#'+this.table_id+'_filter label').remove();
            $('#'+this.table_id+'_filter input').addClass('form-control p-2 mb-2 float-right');
            $('#'+this.table_id+'_filter input').attr("placeholder", "Search table...");
        },
        reset_datatable() {
            ...
            $('#'+this.table_id+'_filter').remove();
        }
    },
    template: /*html*/`
      <table class="table table-sm sortable" :id="this.table_id">
        ...
        <tbody class="list" :id="table_id+'_tbody'">
...
export const vDraftChangesList = {
    ...
    methods: {
        is_new_product(prod_id) {
            const prod_changes_data = this.product_changes[prod_id]
        ...
        is_new_component(comp_id) {
            const comp_changes_data = this.component_changes[comp_id]
        ...
        scroll_to_on_page(event) {
            event.preventDefault();
            let destination = event.target.dataset.scrollToLink
...
export const vPricingDataLoading = {
    /* A div with a bootstrap spinner. */
    name: 'v-pricing-data-loading',
    template: /*html*/`
      <div class="spinner-grow" role="status"></div><span class="ml-3">Loading data...</span>`
}

 export const vPricingErrorDisplay = {
     /* A list of error messages */
     name: 'v-pricing-error-display',
     template: /*html*/`
